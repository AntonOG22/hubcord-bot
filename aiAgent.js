...
// Confirmed correct model name by process of elimination (see aiAutomod.js
// for the full story) — the actual blocker is the Groq account not having
// model access enabled yet, not this model ID. Swap to a 70B-class model
// once the account has broader access, for better tool-calling quality.
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        response: { type: 'string', description: 'What the bot replies with. Supports {user}, {username}, {server}, {membercount} placeholders.' },
        useEmbed: { type: 'boolean', description: 'Optional — post the response as an embed instead of plain text' },
        embedTitle: { type: 'string', description: 'Optional embed title (only shown if useEmbed is true)' },
        color: { type: 'string', description: 'Optional hex color for the embed' },
        imageUrl: { type: 'string', description: 'Optional image URL attached to the embed' },
        allowedRoleIds: { type: 'array', items: { type: 'string' }, description: 'Optional — restrict the command to these roles only. Empty/omitted = everyone can use it.' },
        visibility: { type: 'string', enum: ['public', 'private'], description: '"private" DMs the response to whoever ran it and deletes their trigger message, so nobody else sees it.' },
        cooldownSeconds: { type: 'number', description: 'Optional per-user cooldown in seconds between uses (0 = no cooldown)' },
      },
